...
print("-"*70)
accuracy_bdt = accuracy_score(y_test, y_pred_bagging_dt)
precision_bdt = precision_score(y_test, y_pred_bagging_dt, average='weighted')
recall_bdt = recall_score(y_test, y_pred_bagging_dt, average='weighted')
f1_bdt = f1_score(y_test, y_pred_bagging_dt, average='weighted')

print(f"  准确率 (Accuracy) : {accuracy_bdt:.4f} ({accuracy_bdt*100:.2f}%)")
print(f"  精确率 (Precision): {precision_bdt:.4f}")
print(f"  召回率 (Recall)   : {recall_bdt:.4f}")
print(f"  F1分数 (F1-Score) : {f1_bdt:.4f}")

print("\n详细分类报告:")
print(classification_report(y_test, y_pred_bagging_dt, target_names=target_names))

# 评估Bagging-KNN模型
print("\n【Bagging-KNN模型性能】")
print("-"*70)
accuracy_bknn = accuracy_score(y_test, y_pred_bagging_knn)
precision_bknn = precision_score(y_test, y_pred_bagging_knn, average='weighted')
recall_bknn = recall_score(y_test, y_pred_bagging_knn, average='weighted')
f1_bknn = f1_score(y_test, y_pred_bagging_knn, average='weighted')

print(f"  准确率 (Accuracy) : {accuracy_bknn:.4f} ({accuracy_bknn*100:.2f}%)")
print(f"  精确率 (Precision): {precision_bknn:.4f}")
print(f"  召回率 (Recall)   : {recall_bknn:.4f}")
print(f"  F1分数 (F1-Score) : {f1_bknn:.4f}")

# 对比两种Bagging模型
print("\n【两种Bagging模型对比】")
print("-"*70)
print(f"{'指标':<15s} {'Bagging+DT':<15s} {'Bagging+KNN':<15s} {'差异':<15s}")
print("-"*60)
print(f"{'准确率':<15s} {accuracy_bdt:<15.4f} {accuracy_bknn:<15.4f} {accuracy_bknn-accuracy_bdt:+.4f}")
print(f"{'F1分数':<15s} {f1_bdt:<15.4f} {f1_bknn:<15.4f} {f1_bknn-f1_bdt:+.4f}")

if accuracy_bdt > accuracy_bknn:
    print("\n✓ Bagging-DecisionTree模型性能更优！")
else:
    print("\n✓ Bagging-KNN模型性能更优！")
...
for model_name, y_pred in models_performance.items():
    ...
    prec = precision_score(y_test, y_pred, average='weighted')  # 精确率
    rec = recall_score(y_test, y_pred, average='weighted')  # 召回率
...
for idx, metric in enumerate(metrics):
    ...
    # 绘制柱状图
    bars = ax.bar(
        range(len(results_df)),  # x轴位置
        results_df[metric],  # 柱子高度
        color=colors,  # 颜色
    ...
    ax.set_xticklabels(results_df['模型'], rotation=45, ha='right', fontsize=10)
    
    # 设置y轴标签和标题
    ax.set_ylabel(metric, fontsize=13, fontweight='bold')
    ax.set_title(f'各模型{metric}对比', fontsize=14, fontweight='bold')
    ...
    for i, bar in enumerate(bars):
        ...
        ax.text(
            ...
            height + 0.01,  # y位置：柱子顶部稍上方
            f'{height:.4f}',  # 文本内容：保留4位小数
            ha='center',  # 水平居中
            va='bottom',  # 垂直底部对齐
            fontsize=9,
            fontweight='bold'
...
print("✓ 雷达图对比完成！")
print("\n【雷达图解读】")
print("- 多边形面积越大，模型综合性能越好")
print("- 多边形越接近正方形，说明各项指标越均衡")
...
print("\n【交叉验证原理】")
print("-"*70)
print("k折交叉验证将训练数据分为k份：")
print("  1. 每次使用k-1份作为训练集，1份作为验证集")
print("  2. 重复k次，每份数据都被用作验证集一次")
print("  3. 计算k次验证结果的平均值和标准差")
print("\n优点：更全面地评估模型性能，减少偶然性")
...
for model_name, model in cv_models.items():
    ...
    cv_results.append({
        '模型': model_name,
        '平均准确率': cv_scores.mean(),  # 5次验证的平均准确率
        '标准差': cv_scores.std(),  # 标准差，反映稳定性
        '最小值': cv_scores.min(),  # 最差的一次
        '最大值': cv_scores.max()  # 最好的一次
    })
...
cv_results_df = cv_results_df.sort_values('平均准确率', ascending=False)
...
print("\n【结果解读】")
print("-"*70)
print("标准差越小，说明模型性能越稳定：")
for _, row in cv_results_df.iterrows():
    stability = "非常稳定" if row['标准差'] < 0.01 else "稳定" if row['标准差'] < 0.02 else "一般"
    print(f"  {row['模型']:<20s}: {stability} (σ={row['标准差']:.4f})")
...
bars = ax.bar(
    ...
    alpha=0.7,  # 透明度
    capsize=10,  # 误差条帽的大小
    color=['#1f77b4', '#ff7f0e', '#2ca02c'],  # 颜色
    edgecolor='black',  # 边框颜色
    linewidth=1.5,  # 边框宽度
    error_kw={'linewidth': 2, 'ecolor': 'red'}  # 误差条样式
)

# 设置轴标签和标题
ax.set_xlabel('模型', fontsize=13, fontweight='bold')
ax.set_ylabel('准确率', fontsize=13, fontweight='bold')
ax.set_title('交叉验证准确率对比（5折）', fontsize=15, fontweight='bold', pad=20)

# 设置x轴刻度
ax.set_xticks(x_pos)
ax.set_xticklabels(cv_results_df['模型'], fontsize=11)
...
# ### 5.3 ROC曲线与AUC分析
# 
# ROC曲线（Receiver Operating Characteristic Curve）展示了不同阈值下模型的真正率和假正率。
# AUC（Area Under Curve）是ROC曲线下的面积，AUC越大表示模型性能越好。
...
print("-"*70)
print("ROC曲线：")
print("  - 横轴：假正率(FPR) = 假正例 / (假正例 + 真负例)")
print("  - 纵轴：真正率(TPR) = 真正例 / (真正例 + 假负例)")
print("  - 对角线：随机猜测的基准线")
...
# 将标签二值化（用于多分类ROC曲线）
# 将类别1,2,3转换为三个二进制列：[1,0,0], [0,1,0], [0,0,1]
y_test_bin = label_binarize(y_test, classes=[1, 2, 3])
n_classes = y_test_bin.shape[1]  # 类别数量
...
for idx, (model_name, (model, y_score)) in enumerate(models_for_roc.items()):
    ...
    for i, color, class_name in zip(range(n_classes), colors, class_names):
        ax.plot(
            ...
            color=color,  # 颜色
            lw=2,  # 线宽
            label=f'{class_name} (AUC = {roc_auc[i]:.3f})'  # 图例标签
        )
    ...
    ax.set_xlabel('False Positive Rate (假正率)', fontsize=11, fontweight='bold')
    ax.set_ylabel('True Positive Rate (真正率)', fontsize=11, fontweight='bold')
    ax.set_title(f'{model_name}\nROC曲线', fontsize=13, fontweight='bold')
...
print("\n【AUC得分汇总】")
print("="*70)
print(f"{'模型':<20s} {'Normal':<12s} {'Suspect':<12s} {'Pathologic':<12s} {'Macro-Avg':<12s}")
print("-"*70)
...
for model_name, (model, y_score) in models_for_roc.items():
    ...
    # 计算macro-average AUC（宏平均：所有类别AUC的简单平均）
    macro_auc = np.mean(auc_scores)
    
    # 输出结果
    print(f"{model_name:<20s} {auc_scores[0]:<12.4f} {auc_scores[1]:<12.4f} "
          f"{auc_scores[2]:<12.4f} {macro_auc:<12.4f}")

# ==================== OUTPUT ====================
# 运行此单元格，显示：
# - ROC曲线和AUC的原理介绍
# - 三个模型的ROC曲线图（每个模型包含3个类别的曲线）
# - 详细的AUC得分表
...
# ## 6. 学习曲线分析
# 
# 学习曲线展示了模型性能随训练样本数量增加的变化趋势，
# 帮助我们判断模型是否存在过拟合或欠拟合问题。
...
print("学习曲线可以帮助我们：")
print("  1. 判断模型是否过拟合：")
print("     - 训练得分高但验证得分低 → 过拟合")
print("  2. 判断模型是否欠拟合：")
print("     - 训练和验证得分都较低 → 欠拟合")
print("  3. 判断是否需要更多数据：")
print("     - 两条曲线持续上升且未收敛 → 需要更多数据")
print("     - 两条曲线已经平稳 → 更多数据帮助有限")
...
for idx, (model_name, model) in enumerate(models_for_learning_curve.items()):
    ...
    print("（这可能需要一些时间，因为需要多次训练模型）")
    
    # 计算学习曲线
    # train_sizes: 训练集大小的比例序列
    # 从10%到100%，分10个点
    train_sizes, train_scores, val_scores = learning_curve(
        ...
        cv=5,  # 5折交叉验证
        n_jobs=-1,  # 并行计算
        train_sizes=np.linspace(0.1, 1.0, 10),  # 训练集大小序列
        scoring='accuracy',  # 评估指标
        random_state=42
    )
    
    # 计算均值和标准差
    # train_scores和val_scores是2D数组：(n_train_sizes, n_cv_folds)
    train_mean = np.mean(train_scores, axis=1)  # 训练得分的均值
    train_std = np.std(train_scores, axis=1)  # 训练得分的标准差
    val_mean = np.mean(val_scores, axis=1)  # 验证得分的均值
    ...
    # 绘制训练得分曲线
    ax.plot(
        train_sizes, train_mean# ==================== MARKDOWN ====================
